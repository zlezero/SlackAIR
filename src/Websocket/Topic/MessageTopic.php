...
class MessageTopic implements TopicInterface, SecuredTopicInterface
{
    ...
    private function broadcastMessage(Topic $topic, Message $message, User $user, bool $system, Groupe $groupe) {
        $topic->broadcast(['message' => $message->getTexte(), 'messageTime' => date_format($message->getDateEnvoi(), 'r'),'pseudo' => $system ? "SYSTEM" : $user->getPseudo(), 'clientId' => $system ? null : $user->getId(), 'system' => $system, 'channel' => $groupe->getId()]);
    }
